...
export default function EventCard({ event }) {
  ...
  ---------------------------- */
  const [dynamicViewCount, setDynamicViewCount] = useState(
    typeof event.baseViewCount === "number"
      ? event.baseViewCount
      : null
  );

  useEffect(() => {
    if (dynamicViewCount === null) return;

    const interval = setInterval(() => {
      // Random increase between 1–12 views
      const randomIncrease = Math.floor(Math.random() * 12) + 1;
      setDynamicViewCount((prev) => prev + randomIncrease);
    }, 3 * 1 * 1000); // every 3 minutes

    return () => clearInterval(interval);
  }, [dynamicViewCount]);

  const viewCount = formatViewCount(dynamicViewCount);
